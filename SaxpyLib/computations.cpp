...
int verifyOutput(float* first, float* second, unsigned int size) {
	for(unsigned int i = 0; i < size; i++) {
		if(first[i] != second[i])
			return -1;
	}
...
DWORD WINAPI handleComputation(void* data) {
	...
	cl_mem_flags src_flags = h_data->profiling_data->src_flags[device_index];
	cl_mem_flags dst_flags = h_data->profiling_data->dst_flags[device_index];
	int map_src = h_data->profiling_data->map_src[device_index];
	int map_dst = h_data->profiling_data->map_dst[device_index];
